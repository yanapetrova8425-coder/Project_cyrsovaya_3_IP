...
// ============================================
// GET-запрос — возвращаю данные из всех таблиц
// ============================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['email']) && isset($_POST['password']) && !isset($_POST['name'])) {
        try {
            // Получаю email и пароль
            $email = $_POST['email'];
            $password = $_POST['password'];

            // Ищу пользователя в базе по email
            $stmt = $pdo->prepare("SELECT id, name, email, password FROM users WHERE email = ?");
            $stmt->execute([$email]);
            $user = $stmt->fetch();

            // Сравниваю введённый пароль с тем, что в базе (без хеширования)
    ...
    if (isset($_POST['client_name']) && isset($_POST['client_phone']) && isset($_POST['service']) && isset($_POST['booking_date']) && isset($_POST['booking_time'])) {
        try {
            // Собираю все данные формы записи
    ...
    if (isset($_POST['client_name']) && isset($_POST['review_text'])) {
        try {
            // Получаю имя, текст отзыва и оценку
            $clientName = $_POST['client_name'];
            $reviewText = $_POST['review_text'];
            $rating = isset($_POST['rating']) ? intval($_POST['rating']) : 5;

            // Сохраняю отзыв в базу
...
// Если запрос не POST — возвращаю ошибку
